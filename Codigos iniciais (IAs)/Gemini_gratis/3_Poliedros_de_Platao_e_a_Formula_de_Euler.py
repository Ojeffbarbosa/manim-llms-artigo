...
from manim.mobject.three_d.polyhedra import (
    TETRAHEDRON_VERTICES, TETRAHEDRON_FACES,
    OCTAHEDRON_VERTICES, OCTAHEDRON_FACES,
    DODECAHEDRON_VERTICES, DODECAHEDRON_FACES,
    ICOSAHEDRON_VERTICES, ICOSAHEDRON_FACES
)
...
class PlatonicSolidsEuler(ThreeDScene):
    def construct(self):
        ...
        solids_data = {
            "Tetraedro": {"solid_key": "TETRAHEDRON", "V": 4, "A": 6, "F": 4},
            "Cubo": {"solid_key": "CUBE", "V": 8, "A": 12, "F": 6},
            "Octaedro": {"solid_key": "OCTAHEDRON", "V": 6, "A": 12, "F": 8},
            "Dodecaedro": {"solid_key": "DODECAHEDRON", "V": 20, "A": 30, "F": 12},
            "Icosaedro": {"solid_key": "ICOSAHEDRON", "V": 12, "A": 30, "F": 20},
        }
        ...
        for solid_name, data in solids_data.items():
            ...
            if solid_key == "CUBE":
                solid_mobject = Cube(side_length=2)
            else:
                # Usa os vértices e faces predefinidos pelo Manim para poliedros
                vertices = globals()[f"{solid_key}_VERTICES"]
                faces = globals()[f"{solid_key}_FACES"]
                solid_mobject = Polyhedron(vertices, faces)
            ...
            # Adiciona um atualizador para rotação contínua
            self.add(solid_mobject.always_rotate(axis=UP, rate=PI/5))
            self.wait(1)
